 ...
 */
final class Version20200331145111 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, email, roles, password, name, image_url, address, latitude, longitude FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, email VARCHAR(180) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, image_url VARCHAR(500) DEFAULT NULL COLLATE BINARY, address VARCHAR(500) DEFAULT NULL COLLATE BINARY, latitude DOUBLE PRECISION DEFAULT NULL, longitude DOUBLE PRECISION DEFAULT NULL, roles CLOB NOT NULL --(DC2Type:json)
        , name VARCHAR(255) DEFAULT NULL, city VARCHAR(255) DEFAULT NULL, postal_code INTEGER DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, email, roles, password, name, image_url, address, latitude, longitude) SELECT id, email, roles, password, name, image_url, address, latitude, longitude FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, email, roles, password, name, image_url, address, latitude, longitude FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, email VARCHAR(180) NOT NULL, password VARCHAR(255) NOT NULL, image_url VARCHAR(500) DEFAULT NULL, address VARCHAR(500) DEFAULT NULL, latitude DOUBLE PRECISION DEFAULT NULL, longitude DOUBLE PRECISION DEFAULT NULL, roles CLOB NOT NULL COLLATE BINARY, name VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO user (id, email, roles, password, name, image_url, address, latitude, longitude) SELECT id, email, roles, password, name, image_url, address, latitude, longitude FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649E7927C74 ON user (email)');
        $this->addSql('DROP INDEX IDX_F7129A803AD8644E');
