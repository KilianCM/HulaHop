 ...
 */
final class Version20200331154042 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, email, password, image_url, address, latitude, longitude, roles, name, city, postal_code FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, email VARCHAR(180) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, image_url VARCHAR(500) DEFAULT NULL COLLATE BINARY, address VARCHAR(500) DEFAULT NULL COLLATE BINARY, latitude DOUBLE PRECISION DEFAULT NULL, longitude DOUBLE PRECISION DEFAULT NULL, roles CLOB NOT NULL COLLATE BINARY --(DC2Type:json)
        , name VARCHAR(255) DEFAULT NULL COLLATE BINARY, city VARCHAR(255) DEFAULT NULL COLLATE BINARY, postal_code VARCHAR(5) DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, email, password, image_url, address, latitude, longitude, roles, name, city, postal_code) SELECT id, email, password, image_url, address, latitude, longitude, roles, name, city, postal_code FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE INDEX IDX_D8892622E48FD905 ON rating (game_id)');
        $this->addSql('DROP INDEX UNIQ_8D93D649E7927C74');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, email, roles, password, name, image_url, address, latitude, longitude, city, postal_code FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, email VARCHAR(180) NOT NULL, roles CLOB NOT NULL --(DC2Type:json)
        , password VARCHAR(255) NOT NULL, name VARCHAR(255) DEFAULT NULL, image_url VARCHAR(500) DEFAULT NULL, address VARCHAR(500) DEFAULT NULL, latitude DOUBLE PRECISION DEFAULT NULL, longitude DOUBLE PRECISION DEFAULT NULL, city VARCHAR(255) DEFAULT NULL, postal_code INTEGER DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, email, roles, password, name, image_url, address, latitude, longitude, city, postal_code) SELECT id, email, roles, password, name, image_url, address, latitude, longitude, city, postal_code FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649E7927C74 ON user (email)');
        $this->addSql('DROP INDEX IDX_F7129A803AD8644E');
        $this->addSql('DROP INDEX IDX_F7129A80233D34C1');
